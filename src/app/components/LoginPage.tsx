...
import { Mail, Lock, Eye, EyeOff, LogIn, Send, X } from "lucide-react";
      ...
      {step === "email" ? (
        ...
        <div className="p-5 flex flex-col items-center text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">We sent a reset link to <strong className="text-[#0d2657]">{resetEmail}</strong>.</p>
          </div>
          <button onClick={handleClose} className="w-full bg-[#0d2657] text-white py-3 rounded-xl font-semibold text-sm hover:bg-[#1a3a6b] transition-all">Back to Sign In</button>
        </div>
      )}
    </Modal>
  );
...
function SSOModal({ open, onClose, onSuccess }: { open: boolean; onClose: () => void; onSuccess: () => void }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Single Sign-On">
      <div className="p-5 space-y-4">
        <p className="text-sm text-gray-500">Enter your organization's ID or domain to continue with SSO.</p>
        <div>
          ...
          <div className={`flex items-center border rounded-lg px-3 py-2.5 gap-2 focus-within:border-[#1a5cdb] transition-colors ${error ? "border-red-400" : "border-gray-300"}`}>
            ...
            <input type="text" placeholder="e.g. university.edu or ORG-12345" value={orgId}
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-3">
          <p className="text-xs text-blue-700"><strong>Note:</strong> You will be redirected to your organization's login page.</p>
        </div>
        <button onClick={handleContinue} disabled={loading}
          className="w-full bg-[#0d2657] hover:bg-[#1a3a6b] text-white py-3 rounded-xl flex items-center justify-center gap-2 font-semibold text-sm transition-all disabled:opacity-70">
          {loading ? <svg className="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" /></svg> : "Continue with SSO"}
        </button>
      </div>
    </Modal>
  );
...
function RequestAccessModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Request Access" size="lg">
      {!submitted ? (
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {(["name", "email"] as const).map((key) => (
              <div key={key}>
                <label className="text-sm text-gray-600 mb-1 block">{key === "name" ? "Full Name" : "Email Address"}</label>
                <div className={`flex items-center border rounded-lg px-3 py-2.5 gap-2 focus-within:border-[#1a5cdb] transition-colors ${errors[key] ? "border-red-400" : "border-gray-300"}`}>
                  <input type={key === "email" ? "email" : "text"} placeholder={key === "name" ? "John Doe" : "john@university.edu"}
          ...
          <div>
            ...
            <textarea value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })}
              placeholder="Briefly describe why you need access…" rows={3}
          ...
          <button onClick={handleSubmit} disabled={loading}
            className="w-full bg-[#0d2657] hover:bg-[#1a3a6b] text-white py-3 rounded-xl flex items-center justify-center gap-2 font-semibold text-sm transition-all disabled:opacity-70">
            {loading ? <svg className="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" /></svg> : "Submit Request"}
          </button>
        </div>
      ) : (
        <div className="p-5 flex flex-col items-center text-center space-y-4">
          ...
          <button onClick={handleClose} className="w-full bg-[#0d2657] text-white py-3 rounded-xl font-semibold text-sm hover:bg-[#1a3a6b] transition-all">Done</button>
        </div>
      )}
    </Modal>
  );
  ...
  return (
    <Modal open={!!topic} onClose={onClose} title={info.title}>
      <div className="p-5">
        ...
        <button onClick={onClose} className="mt-5 w-full bg-[#0d2657] text-white py-3 rounded-xl font-semibold text-sm hover:bg-[#1a3a6b] transition-all">Close</button>
      </div>
    </Modal>
  );
}

/* ─── LoginPage (Desktop Split-Screen) ───────────────────────── */
  ...
  return (
    <>
      ...
      <div className="min-h-screen flex">
        {/* ── Left Panel: Hero ── */}
        <div className="hidden lg:flex lg:w-[55%] xl:w-[60%] relative flex-col">
          <img src={HERO_IMAGE} alt="Office" className="absolute inset-0 w-full h-full object-cover" />
          <div className="absolute inset-0" style={{ background: "linear-gradient(135deg, #0d1e3d 0%, rgba(13,38,87,0.85) 50%, rgba(26,58,107,0.75) 100%)" }} />
          ...
          <div className="relative z-10 flex flex-col h-full px-12 py-10">
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-white/20 backdrop-blur-sm rounded-xl flex items-center justify-center">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <rect x="2" y="5" width="20" height="14" rx="2" /><path d="M8 5V3h8v2M12 12a2 2 0 1 0 0-4 2 2 0 0 0 0 4M8 17a4 4 0 0 1 8 0" />
                </svg>
              </div>
              <span className="text-white font-bold text-xl tracking-wide">Career Passport</span>
            </div>
            ...
            <div className="flex-1 flex flex-col justify-center max-w-lg">
              ...
              <p className="text-gray-300 text-lg leading-relaxed mb-8">
                Connect with employers, track your applications, build your network, and generate a professional CV — all from your Career Passport.
              </p>

              {/* Feature highlights */}
              <div className="space-y-3">
                {[
                  { icon: "🎓", text: "Verified alumni credentials & academic records" },
                  { icon: "💼", text: "AI-matched job listings tailored to your profile" },
                  { icon: "🤝", text: "Professional networking with industry leaders" },
                ].map((f) => (
            ...
            <div className="flex gap-8">
              {[{ value: "50k+", label: "Alumni" }, { value: "3,200+", label: "Employers" }, { value: "98%", label: "Placement Rate" }].map((s) => (
                <div key={s.label}>
                  <p className="text-white font-bold text-2xl">{s.value}</p>
                  <p className="text-gray-400 text-xs font-medium tracking-wider uppercase">{s.label}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col bg-white overflow-y-auto">
          <div className="flex-1 flex flex-col justify-center px-10 py-12 max-w-md mx-auto w-full">
            ...
            <div className="lg:hidden mb-8">
              <span className="text-[#1a3a6b] font-bold text-2xl">Career Passport</span>
            </div>

            <div className="mb-8">
              <h2 className="text-[#0d2657] text-3xl font-bold mb-2">Welcome back</h2>
              <p className="text-gray-500 text-sm">Sign in to your account to continue</p>
            </div>

            {/* Tab Switcher */}
            <div className="border border-[#1a5cdb] rounded-xl grid grid-cols-3 overflow-hidden mb-6">
              {TABS.map((tab) => (
                <button key={tab} onClick={() => setActiveTab(tab)}
                  className={`py-2.5 text-sm font-semibold transition-colors ${activeTab === tab ? "bg-[#0d2657] text-white" : "text-gray-500 hover:text-[#1a5cdb] bg-white"}`}>
                  {tab}
                </button>
              ))}
            </div>
            ...
            <div className="space-y-4">
              {/* Email */}
              <div>
                <label className="text-sm text-gray-600 mb-1.5 block">Email or ID</label>
                <div className={`flex items-center border rounded-xl px-4 py-3 gap-2 bg-white transition-colors ${emailError ? "border-red-400" : "border-gray-200 focus-within:border-[#1a5cdb]"}`}>
                  <Mail size={16} className="text-gray-400 shrink-0" />
                  <input type="email" placeholder="Enter your email or ID" value={email}
              ...
              <div>
                ...
                <div className="flex items-center border border-gray-200 rounded-xl px-4 py-3 gap-2 bg-white focus-within:border-[#1a5cdb] transition-colors">
              ...
              <div className="flex justify-end">
                <button onClick={() => setShowForgot(true)} className="text-[#1a5cdb] text-sm hover:underline">Forgot Password?</button>
              </div>

              {/* Sign In */}
              <button onClick={handleSignIn} disabled={loading || googleLoading}
              ...
              <button onClick={handleGoogle} disabled={googleLoading || loading}
                className="w-full border border-gray-200 rounded-xl py-3 flex items-center justify-center gap-3 text-sm text-gray-700 hover:bg-gray-50 active:scale-[0.98] transition-all disabled:opacity-70">
                {googleLoading
              ...
              {/* SSO */}
              <button onClick={() => setShowSSO(true)} disabled={loading || googleLoading}
                className="w-full border border-gray-200 rounded-xl py-3 flex items-center justify-center gap-3 text-sm text-gray-700 hover:bg-gray-50 active:scale-[0.98] transition-all disabled:opacity-70">
            ...
            <div className="mt-8 text-center">
              <p className="text-sm text-gray-500">
                ...
                <button onClick={() => setShowRegister(true)} className="text-[#0d2657] font-bold hover:underline">Request Access</button>
              </p>
              <div className="flex items-center justify-center gap-5 mt-4">
                {["PRIVACY", "TERMS", "SUPPORT"].map((item) => (
                  <button key={item} onClick={() => setInfoTopic(item)}
